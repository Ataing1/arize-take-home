...
class RecursivePaperCollector:
    def __init__(
        self, base_dir: str = "data", max_depth: int = 3, max_papers: int = 200
    ):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                author_elems = root.findall(
                    ".//atom:entry/atom:author/atom:name", namespace
                )
    ...
    def get_collection_stats(self) -> Dict:
        ...
        return {
            ...
            "total_citations": sum(
                len(refs) for refs in self.metadata["citations"].values()
            ),
...
async def main():
    ...
    def validate_arxiv_id(value):
        pattern = r"^(?:0[1-9]|[1-9]\d)(?:0[1-9]|1[0-2])\.\d{4,5}(?:v\d+)?$"
        if not re.match(pattern, value):
            raise argparse.ArgumentTypeError(
                f"Invalid arXiv ID format. Must be YYMM.NNNNN where:\n"
                f"YY = year (01-99)\n"
                f"MM = month (01-12)\n"
                f"NNNNN = 4-5 digit number\n"
                f"Example: 2312.12345"
            )
        return value

    parser = argparse.ArgumentParser(
        description="Recursively collect academic papers from arXiv"
    )
    parser.add_argument(
        "--seed-paper",
        type=validate_arxiv_id,
        default="1706.03762",
        help="ArXiv ID of the seed paper in format YYMM.NNNNN (default: 1706.03762)",
    )
    parser.add_argument(
        "--max-depth",
        type=int,
        default=2,
        help="Maximum depth for recursive paper collection (default: 2)",
    )
    parser.add_argument(
        "--max-papers",
        type=int,
    ...
    # Clean the data directory if it exists
    if Path("data").exists():
        shutil.rmtree("data")
        print("Cleaned existing data directory: data")

    collector = RecursivePaperCollector(
        base_dir="data", max_depth=args.max_depth, max_papers=args.max_papers
    )

    await collector.process_paper_recursively(args.seed_paper)
